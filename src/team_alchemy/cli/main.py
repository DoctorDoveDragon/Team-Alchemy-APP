            ...
            if assessment_type == "full":
                ...
                if archetype_patterns:
                    ...
                    for pattern in archetype_patterns[:3]:
                        typer.echo(f"  - {pattern.archetype.value}: {pattern.strength:.1%} strength")
                    typer.echo()
                ...
                if defense_profiles:
                    ...
                    for defense in defense_profiles[:3]:
                        ...
                        typer.echo(f"  - {defense.mechanism.value}: {adaptive} ({defense.adaptiveness:.1%})")
                    typer.echo()
    ...
    try:
        ...
        try:
            ...
            for member in members:
                ...
                if profile and profile.jungian_type:
                    ...
                    try:
                        ...
                        if jungian_profile:
                            for func in jungian_profile.get_function_stack()[:2]:  # Dominant and auxiliary
                                func_name = func.value
                                function_coverage[func_name] = function_coverage.get(func_name, 0) + 1
            ...
            for mbti_type, count in sorted(mbti_distribution.items()):
                ...
                typer.echo(f"  - {mbti_type}: {count} member{'s' if count > 1 else ''} ({percentage:.0f}%)")
            typer.echo()
            
            # Display team dynamics
            typer.echo("=== Team Dynamics ===")
            diversity_score = len(mbti_distribution) / len(members_with_profiles) if members_with_profiles else 0
            ...
            if all_archetypes:
                ...
                typer.echo(f"Archetype Diversity: {collective.get('archetype_diversity', 0)} archetypes")
            ...
            if diversity_score < 0.4:
                typer.echo("- Consider diversifying team composition for better perspective balance")
            
            # Check for function gaps
            all_functions = ["Ti", "Te", "Fi", "Fe", "Si", "Se", "Ni", "Ne"]
            missing_functions = [f for f in all_functions if f not in function_coverage or function_coverage[f] == 0]
            
            if "Se" in missing_functions or "Si" in missing_functions:
                typer.echo("- Team may lack practical execution focus - consider adding sensing-focused members")
            ...
            if similar_cases:
                for case in similar_cases:
                    ...
                    recommendations.append({
                        ...
                        "rationale": f"Similar team benefited from these interventions",
