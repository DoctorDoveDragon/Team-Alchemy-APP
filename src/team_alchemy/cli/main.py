...
def get_user_mbti_type(profile: Optional[Any]) -> tuple[str, bool]:
    ...
    Returns:
        tuple: (mbti_type_string, is_using_default)
    """
...
def display_defense_mechanisms(defense_profiles: list) -> None:
    ...
    if defense_profiles:
        ...
        typer.echo("    (Assessment requires behavioral observations for detailed analysis)")
    typer.echo()
...
def assess(
    ...
    from team_alchemy.data.repository import get_db_session
    ...
    try:
        with get_db_session() as db:
            ...
            # Display user information (always shown)
            display_user_info(user)
            
            # Get user profile
            profile = db.query(UserProfile).filter(UserProfile.user_id == user_id).first()
            
            # Get MBTI type with fallback
            mbti_type_str, is_using_default = get_user_mbti_type(profile)
            
            if is_using_default:
                typer.echo("⚠ Warning: User has no MBTI assessment. Using INTJ as example.")
            ...
            # Display sections based on assessment type
        ...
        logger.error(f"Assessment failed for user {user_id}: {e}", exc_info=True)
    ...
    try:
        with get_db_session() as db:
            ...
            for member in team.members:
                ...
                # Determine MBTI type
                mbti_type_str, _ = get_user_mbti_type(profile)
        ...
        logger.error(f"Team analysis failed for team {team_id}: {e}", exc_info=True)
...
def recommend(
    ...
    try:
        ...
        logger.error(f"Recommendations failed for team {team_id}: {e}", exc_info=True)
